            ...
            return (
                <>
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-2 mb-1.5">
                            <span className="text-[10px] font-medium text-muted-foreground">Sessions for</span>
                            <span className="text-[10px] text-primary bg-primary/10 px-1.5 py-0.5 rounded font-medium">
                                Day {activeAgendaDay + 1}
                            </span>
                        </div>
                        <div className="space-y-4 bg-muted/30 p-3 rounded-md border border-border max-h-[400px] overflow-y-auto">
                            {Array.from({ length: sessionsForDay }, (_, sIdx) => {
                                const speakerCountKey = `speakers-d${activeAgendaDay}-s${sIdx}`;
                                const defaultCount = 2 + ((activeAgendaDay * 5 + sIdx * 3) % 3);
                                const speakerCount = (counts[speakerCountKey] !== undefined) ? counts[speakerCountKey] : defaultCount;
                                const sessionKey = `session-d${activeAgendaDay}-s${sIdx}`;
                                const sessionTitle = (selectedSettings.text && selectedSettings.text[sessionKey]) || `Session ${sIdx + 1}`;

                                return (
                                    <div key={`speakers-session-${sIdx}`} className="space-y-2 pb-3 border-b border-border/50 last:border-0 last:pb-0">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[11px] font-semibold text-foreground truncate max-w-[120px]" title={sessionTitle}>
                                                {sessionTitle}
                                            </span>
                                                    ...
                                                    onClick={() => {
                                                        if (speakerCount > 0) {
                                                            const newCounts = {
                                                                ...
                                                                [speakerCountKey]: speakerCount - 1
                                                            };
                                                            onChangeSettings({ ...selectedSettings, counts: newCounts });
                                                        }
                                                    }}
                                                    className="w-5 h-5 flex items-center justify-center rounded border border-border bg-background text-muted-foreground hover:bg-muted transition-colors"
                                                >
                                                    <Minus className="w-3 h-3" />
                                                </button>
                                                <span className="text-[11px] font-mono font-medium w-4 text-center">{speakerCount}</span>
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        if (speakerCount < 10) {
                                                            ...
                                                            // Auto-set default name for the new speaker
                                                            const nameKey = `speaker-d${activeAgendaDay}-s${sIdx}-${speakerCount}`;
                                                            const defaultName = `Speaker ${speakerCount + 1}`;
                                                            const currentText = selectedSettings.text || {};
                                                            if (!currentText[nameKey]) {
                                                                onChangeSettings({ ...selectedSettings, counts: newCounts, text: { ...currentText, [nameKey]: defaultName } });
                                                            } else {
                                                                onChangeSettings({ ...selectedSettings, counts: newCounts });
                                                            }
                                                        }
                                        ...
                                        {speakerCount > 0 && (
                                            <div className="ml-1 pl-3 border-l-2 border-primary/20 space-y-1.5">
                                                {Array.from({ length: speakerCount }, (_, spIdx) => {
                                                    const nameKey = `speaker-d${activeAgendaDay}-s${sIdx}-${spIdx}`;
                                                    const defaultName = defaultSpeakerNames[(activeAgendaDay * 7 + sIdx * 3 + spIdx) % defaultSpeakerNames.length];
                                                    const currentVal = (selectedSettings.text && selectedSettings.text[nameKey]) || '';
                                                    return (
                                                        <input
                                                            key={nameKey}
                                                            type="text"
